...
Theta1WO =Theta1(:,2:end);
Theta2WO =Theta2(:,2:end);

% Calculate J
%Change square error to tranpsoe matrixes
J = (-1/m) * (sum(sum( (-y).*log(a3)-(1-y).*log(1-a3) )))  + (lambda/(2*m)) * sum(sum(Theta1WO.^2)) + sum(sum(Theta2WO.^2));
